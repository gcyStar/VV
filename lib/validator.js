...
function check(rule, field, value, isArray) {
    ...
    if (Array.isArray(rule)) {
        return rule.map(item => check.call(this, item, field, value, true)).indexOf(false) === -1;
    }
    let vm = this;
    //在实例上定义的校验规则
    ...
    const regex = is('String', rule) ? $rules[rule] : is('String', rule.test || rule.remote) ? $rules[rule.test] : rule;
